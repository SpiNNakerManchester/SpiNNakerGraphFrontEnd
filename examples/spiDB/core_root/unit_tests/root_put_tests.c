...
bool test_receive_sdp_msg(sdp_msg_t* reply_msg){
    ...
    switch(reply_msg->cmd_rc){
        case PUT_REPLY:
            ...
            break;
        case PULL_REPLY:
            log_debug("===== Testing PULL %s =====", reply_msg->data);

            q = get_unreplied_query(unreplied_pulls, reply_msg->seq);
            assert_info(q, "PUT_REPLY of id: %d was not found on unreplied_pulls", reply_msg->seq);
            ...
            uint32_t reply_v_size = reply_msg->arg2;
            ...
            break;
        default:
            sentinel("Received invalid cmd_rc: %d", reply_msg->cmd_rc);
            return false;
            break;
    }
