...
# command sender for injecting fake temps or stop/pause/resume commands to the
# fabric
command_injector = \
...
# gatehrer to reduce the amount of bandwidth on the ethernet.
live_gatherer = \
    front_end.add_partitioned_vertex(
        LivePacketGather,
        ...
        if (y_position - 1) >= 0:
            front_end.add_partitioned_edge(
                ...
                label="South edge between heat elements {}:{}"
                      .format(vertices[x_position][y_position],
                              vertices[x_position][y_position - 1]),)
        else:  # add a command edge for injecting south temps
            front_end.add_partitioned_edge(
                HeatDemoEdge,
                {'pre_subvertex': command_injector,
                 'post_subvertex': vertices[x_position][y_position],
        ...
        if (x_position - 1) >= 0:
            front_end.add_partitioned_edge(
                ...
                label="West edge between heat elements {}:{}"
                      .format(vertices[x_position][y_position],
                              vertices[x_position - 1][y_position]),)
        else:  # add a command edge for injecting west temps
            front_end.add_partitioned_edge(
                HeatDemoEdge,
                {'pre_subvertex': command_injector,
                 'post_subvertex': vertices[x_position][y_position],
