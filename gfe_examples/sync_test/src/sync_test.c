...
static bool initialize(uint32_t *timer_period) {
    ...
    uint32_t *app_data = data_specification_get_region(DATA, data);
    is_sender = app_data[0];
    key = app_data[1];

    log_info("Sender: %u, key %u", is_sender, key);
