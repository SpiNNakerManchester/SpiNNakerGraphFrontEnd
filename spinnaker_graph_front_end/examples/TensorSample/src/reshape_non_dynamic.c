...
void record_data() {
    ...
    spin1_memcpy(record_space_address, tensor1, 4);
    log_info("recorded result %d address: %u\n", tensor1[0] ,record_space_address);
...
static bool initialize() {
    log_info("Initialise Reshape: started\n");
    ...
    if (transmission_region_address[HAS_KEY] == 1) {
        ...
        log_info("Reshape vertex without key, no sending packets");
    }
