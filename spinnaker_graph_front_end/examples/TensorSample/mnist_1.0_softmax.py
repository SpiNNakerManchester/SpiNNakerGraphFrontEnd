...
Y_ = tf.placeholder(tf.float32, [None, 10]) # 1 node 68
# weights W[784, 10]   784=28*28
W = tf.Variable(tf.zeros([784, 10])) # 6 nodes 69 - 74
# biases b[10]
b = tf.Variable(tf.zeros([10])) # 4 nodes 75 - 78

# flatten the images into a single line of pixels
# -1 in the shape definition means "the only possible dimension that will preserve the number of elements"
XX = tf.reshape(X, [-1, 784]) # 2 nodes 79 - 80



# The model
Y = tf.nn.softmax(tf.matmul(XX, W) + b) # 3 nodes 81-83
