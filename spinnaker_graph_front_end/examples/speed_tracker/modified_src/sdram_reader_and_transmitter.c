// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/sdram_reader_and_transmitter.c
...
void sdp(uint mailbox, uint port){
    ...
    log_mini_info("%u", 13001);  /* "starting transfer"*/
    read();
}
...
void write_data(){
    ...
    log_mini_info("%u%d", 13002, iterations - 1);  /* "iterations = %d"*/
...
static bool initialize(uint32_t *timer_period) {
    log_mini_info("%u", 13003);  /* "Initialise: started\n"*/
    ...
    if (!data_specification_read_header(dsg_main_address)) {
        log_mini_error("%u", 13004);  /* "failed to read the data spec header"*/
        return false;
    }
    ...
    log_mini_info("%u%u", 13005, bytes_to_write);  /* "bytes to write is %u"*/
    log_mini_info("%u%u", 13006, key);  /* "my key is %u"*/
 ...
 */
void c_main() {
    log_mini_info("%u", 13007);  /* "starting heat_demo\n"*/
    ...
    // start execution
    log_mini_info("%u", 13008);  /* "Starting\n"*/
