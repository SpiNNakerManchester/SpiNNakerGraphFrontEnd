...
void dma_complete_reading_retransmission_data(uint unused, uint unused2){
    ...
    log_mini_info("%u", 16001);  /* "doing retransmission !!!!!!"*/
...
void sdp_reception(uint mailbox, uint port){
    ...
    // start or continue to gather missing packet list
        ...
        log_mini_error("%u", 16002);  /* "received unknown sdp packet"*/
    }
}
...
void write_data(){
    ...
    if (store_address == NULL){
        log_mini_error("%u", 16003);  /* "failed to xalloc sdram for writing"*/
        rt_error(RTE_SWERR);
    }
...
static bool initialize(uint32_t *timer_period) {
    log_mini_info("%u", 16004);  /* "Initialise: started\n"*/
    ...
    if (!data_specification_read_header(address)) {
        log_mini_error("%u", 16005);  /* "failed to read the data spec header"*/
        return false;
    }
    ...
    log_mini_info("%u%d", 16006, bytes_to_write);  /* "bytes to write is %d"*/
...
void c_main() {
    ...
    log_mini_info("%u", 16007);  /* "starting sdram reader and writer\n"*/
    ...
    // start execution
    log_mini_info("%u", 16008);  /* "Starting\n"*/
