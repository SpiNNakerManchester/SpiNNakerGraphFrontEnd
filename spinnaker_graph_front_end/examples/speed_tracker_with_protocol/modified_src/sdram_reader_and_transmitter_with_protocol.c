...
void sdp_reception(uint mailbox, uint port){
    ...
    if(msg->data[0] == SDP_COMMAND_FOR_SENDING_DATA){
        log_mini_info("%u", 15001);  /* "starting the send of orginial data"*/
    ...
    // start or continue to gather missing packet list
        ...
        log_mini_error("%u", 15002);  /* "received unknown sdp packet"*/
    }
}
...
static bool initialize(uint32_t *timer_period) {
    log_mini_info("%u", 15003);  /* "Initialise: started\n"*/
    ...
    if (!data_specification_read_header(address)) {
        log_mini_error("%u", 15004);  /* "failed to read the data spec header"*/
        return false;
    }
    ...
    log_mini_info("%u%d", 15005, bytes_to_write);  /* "bytes to write is %d"*/
...
void c_main() {
    ...
    log_mini_info("%u", 15006);  /* "starting sdram reader and writer\n"*/
    ...
    // start execution
    log_mini_info("%u", 15007);  /* "Starting\n"*/
