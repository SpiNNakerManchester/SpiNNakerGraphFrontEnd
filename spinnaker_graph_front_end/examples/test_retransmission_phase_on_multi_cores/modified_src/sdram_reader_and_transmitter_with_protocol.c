// DO NOT EDIT! THIS FILE WAS GENERATED FROM src/sdram_reader_and_transmitter_with_protocol.c
...
void the_dma_complete_read_missing_seqeuence_nums(uint unused, uint unused2){
    ...
    if (position_in_read_data > ITEMS_PER_DATA_PACKET){
        ...
        log_mini_info("%u%d", 9001, missing_seq_num_being_processed);  /* "handling missing seq num %d"*/
...
void dma_complete_reading_retransmission_data(uint unused, uint unused2){
    ...
    log_mini_info("%u", 9002);  /* "doing retransmission !!!!!!"*/
...
void sdp_reception(uint mailbox, uint port){
    ...
    // start or continue to gather missing packet list
        ...
        log_mini_error("%u", 9003);  /* "received unknown sdp packet"*/
    }
}
...
void write_data(){
    ...
    if (store_address == NULL){
        log_mini_error("%u", 9004);  /* "failed to xalloc sdram for writing"*/
        rt_error(RTE_SWERR);
    }
...
static bool initialize(uint32_t *timer_period) {
    log_mini_info("%u", 9005);  /* "Initialise: started\n"*/
    ...
    if (!data_specification_read_header(address)) {
        log_mini_error("%u", 9006);  /* "failed to read the data spec header"*/
        return false;
    }
    ...
    log_mini_info("%u%d", 9007, bytes_to_write);  /* "bytes to write is %d"*/
...
void c_main() {
    ...
    log_mini_info("%u", 9008);  /* "starting sdram reader and writer\n"*/
